 ...
 */
class Title implements \TYPO3\CMS\Vidi\GridRenderer\GridRendererInterface {
	...
	public function render(\TYPO3\CMS\Vidi\Domain\Model\Content $asset = NULL) {
		...
		if (!empty($languages) && $asset->getUid() > 0) {
			...
			foreach ($languages as $language) {
				$records = \TYPO3\CMS\Vidi\Utility\Overlays::getOverlayRecords('sys_file', array($asset->getUid()), $language['uid']);
