...
class ThumbnailViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
	...
	public function render($object, $configuration = array(), $attributes = array(), $wrap = FALSE, $preset = NULL,
		...
		/** @var $object \TYPO3\CMS\Vidi\Domain\Model\Content */
		if ($preset) {
			$imageDimension = \TYPO3\CMS\Vidi\Utility\SettingImagePreset::getInstance()->preset($preset);
		...
		if ($wrap) {
			$output = \TYPO3\CMS\Vidi\Service\ThumbnailInterface::OUTPUT_IMAGE_WRAPPED;
		}

		/** @var $thumbnailService \TYPO3\CMS\Vidi\Service\ThumbnailService */
		$thumbnailService = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\CMS\Vidi\Service\ThumbnailService');
