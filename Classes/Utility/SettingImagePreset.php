...
class SettingImagePreset implements \TYPO3\CMS\Core\SingletonInterface {
	 ...
	 *
	 * @return \TYPO3\CMS\Vidi\Utility\SettingImagePreset
	 */
	static public function getInstance() {
		return \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\CMS\Vidi\Utility\SettingImagePreset');
	}
	 ...
	 *
	 * @throws \TYPO3\CMS\Vidi\Exception\EmptyValueException
	 * @param string $preset image_thumbnail, image_mini, ...
	 * @return \TYPO3\CMS\Vidi\Utility\SettingImagePreset
	 */
	public function preset($preset){
		$size = \TYPO3\CMS\Vidi\Utility\Setting::getInstance()->get($preset);
		if (is_null($size)) {
			throw new \TYPO3\CMS\Vidi\Exception\EmptyValueException('No value for preset: ' . $preset, 1362501066);
		}
	...
	public function getWidth(){
		if (empty($this->store[$this->currentPreset])) {
			throw new \TYPO3\CMS\Vidi\Exception\InvalidKeyInArrayException('No existing values for current preset. Have you set a preset?', 1362501853);
		}
